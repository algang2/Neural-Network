...
Tensor<double> LeakyReLU::activation(const Tensor<double>& ten_)
{
	int size = ten_.size();
	Tensor<double> ten(ten_);
	for (int i = 0; i < size; i++)
	{
		if (ten_.element(i) > 0) ten(i) = ten_.element(i);
		else ten(i) = 0.01 * ten_.element(i);
	}
	return ten;
}

Tensor<double> LeakyReLU::deactivation(const Tensor<double>& ten_)
{
	int size = ten_.size();
	Tensor<double> ten(ten_);
	for (int i = 0; i < size; i++)
